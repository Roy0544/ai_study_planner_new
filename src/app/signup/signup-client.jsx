  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || "Sign up failed. Please check your inputs.");
      }
    } catch (err) {
        ...
        <div className="relative z-10 w-full">
          <div className="bg-app-card border border-app-border p-10 rounded-2xl shadow-xl space-y-6 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <div>
                <p className="text-xs font-bold text-text-primary">Nelson Mandela</p>
                <p className="text-[10px] text-text-muted font-mono uppercase tracking-wider">Former President of South Africa</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="relative bg-app-card border border-app-border rounded-2xl p-8 sm:p-10 shadow-xl overflow-hidden">
            ...
            <div className="space-y-6 relative z-10">
              <div className="text-center lg:text-left space-y-1">
                <h2 className="text-2xl font-black tracking-tight text-text-primary" data-display="true">Create an Account</h2>
                <p className="text-text-secondary text-xs">Start your intelligent learning journey today.</p>
              </div>

              {isSuccess ? (
                <div className="space-y-4 animate-in fade-in zoom-in-95 duration-500">
                  <div className="bg-app-brand/10 border border-app-brand/20 p-6 rounded-2xl text-center space-y-3">
                    <div className="w-10 h-10 bg-app-brand/20 rounded-full flex items-center justify-center mx-auto">
                      <span className="material-symbols-outlined text-app-brand text-lg">mail</span>
                    </div>
                    <h3 className="text-base font-bold text-text-primary" data-display="true">Check your email</h3>
                    <p className="text-xs text-text-secondary">
                      We&apos;ve sent a verification link to your email address. Please verify your account to continue.
                    </p>
                  </div>
                  <Button variant="outline" className="w-full h-11 rounded-xl font-bold border border-app-border hover:bg-white/5 text-xs" asChild>
                  ...
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="space-y-1.5">
                            <FormLabel className="text-xs font-bold text-text-primary uppercase tracking-wider ml-0.5">Password</FormLabel>
                            <FormControl>
                              <div className="relative">
                                <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-text-muted text-lg">
                                  lock
              ...
              <p className="text-center text-xs text-text-muted pt-2">
                ...
                <Link href="/login" className="font-bold text-app-brand hover:underline underline-offset-4">Sign in</Link>
              </p>
            </div>
